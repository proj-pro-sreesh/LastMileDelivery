...
- Failed deliveries: entering FAILED increments `orders.delivery_attempt` (inside `order_service.update_status`); FAILED→PENDING goes ONLY through the reschedule flow (`order_service.reschedule_order`: clears assignment, releases agent, bumps scheduled date default tomorrow, tracking row, notification; past date → 422) — the generic admin override rejects that edge with a pointer to the reschedule endpoint. Both `POST /orders/{id}/reschedule` (owner CUSTOMER, 404 for others, role-gated) and `POST /admin/orders/{id}/reschedule` call it with `actor_role`; CUSTOMER path also bumps `delivery_attempt` and auto-reassigns via `assignment_service.auto_assign`. Every reschedule inserts an audit row into `reschedules` (model `app/models/reschedule.py`, migration 5e49f4312ead).
- Admin order filters: `GET /orders?status=&zone_id=&agent_id=` — zone matches pickup OR drop zone (`Order.pickup_zone_id/drop_zone_id`); zone/agent params are admin-only (403 otherwise), implemented in `order_service.list_orders`.
- Notifications: `notification_service.notify*` helpers write in-app rows + dispatch email/SMS via swappable providers (`app/services/providers/email.py`: mock/Resend; `sms.py`: mock/Twilio — stdlib urllib only, no new deps). Settings: `EMAIL_PROVIDER/EMAIL_API_KEY/EMAIL_FROM`, `SMS_PROVIDER/SMS_API_KEY` (`AccountSid:AuthToken`)/`SMS_FROM`; `NOTIFICATIONS_MODE=disabled` still turns everything off. Persistence runs in a SAVEPOINT (`begin_nested`) and both dispatch call-sites are individually failure-isolated — provider crashes must never roll back order transactions. Hooks live in `order_service.update_status`, `reschedule_order`, and `assignment_service._apply_assignment` (customer + agent on ASSIGNED). Feed: `GET /notifications` (?unread=true), `POST /notifications/{id}/read`, `POST /notifications/read-all` — all authenticated, owner-scoped.
- Hosted deploys: `render.yaml` blueprint at repo root (free web service + managed PG, `/health` check, `alembic upgrade head && uvicorn` start). Render hands out `postgres://` URLs — config normalizes to `postgresql+psycopg://...?sslmode=require` (see `_normalize_database_url`, tested in test_config.py). Guides: docs/deployment.md (hosting steps), docs/system-design.md (≤800-word architecture write-up).
- Frontend (`frontend/`): Next.js 16 App Router + TS + Tailwind v4; run `npm run dev` there (port 3000); talks to `NEXT_PUBLIC_API_URL` (default http://localhost:8000). JWT lives in localStorage (`lmt.token`), shared via `components/AuthProvider.tsx`; all HTTP goes through `lib/api.ts`. Routes: `/login`, `/register`, customer `/orders` (+`/orders/[id]` timeline), agent `/agent`, admin `/admin` and `/admin/agents`. Role gating in UI is UX only — the backend enforces authorization.
- Deployment: `docker compose up -d --build` at repo root (postgres:18-alpine + backend auto-migrating on boot + frontend). PG18 images mount volumes at `/var/lib/postgresql`, NOT `.../data`. `NEXT_PUBLIC_API_URL` is baked at frontend build time → pass as compose build arg. Seed inside Docker: `docker compose exec backend python scripts/seed.py`. README.md is the user-facing doc — keep it in sync when behavior changes.
