...
- Notifications: `notification_service.notify*` helpers write in-app rows + log `[MOCK EMAIL]`/`[MOCK SMS]` (mock mode is the default; `NOTIFICATIONS_MODE=disabled` turns everything off). Hooks live in `order_service.update_status`, `reschedule_order`, and `assignment_service._apply_assignment` (customer + agent on ASSIGNED). Feed: `GET /notifications` (?unread=true), `POST /notifications/{id}/read`, `POST /notifications/read-all` — all authenticated, owner-scoped.
